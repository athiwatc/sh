...
DataFilter = (function() {
  ...
  DataFilter.prototype.getAllActivitiesInUnixTime = function() {
      ...
      if (prefix === 0) {
        if (state === state_none) {
          if (tmp[3].toLowerCase() === 'begin') {
            ...
            activity_time = this._floorSecondUnixTime(parseInt(tmp[1]));
          } else {
            time_stack.push({
              name: 'none',
              color: 1,
          ...
          if (tmp[3].toLowerCase() === 'end' && tmp[2] === activity_name) {
            current_time = this._floorSecondUnixTime(parseInt(tmp[1]));
  ...
  DataFilter.prototype._floorSecondUnixTime = function(unixtime) {
...
VisualizeParser = (function() {
  ...
  VisualizeParser.prototype.parseChordDiagram = function(data) {
    ...
    return fs.writeFileSync('./public/' + path_data, data_text);
  };

  VisualizeParser.prototype.parseTimeLineMatrix = function(data) {
    var obj, path, text, _i, _len;
...
Meteor.methods({
  print: function() {
    var c, data_timeline, filter, sd, vp;
