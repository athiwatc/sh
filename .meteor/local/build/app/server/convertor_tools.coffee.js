var Convertor, ConvertorTimelineFormat, DataProcessor, Map, SensorDictionary, Set, VisualizationParser, _getConvertorTimeLineFormat, _ref,
...
DataProcessor = (function() {
  ...
  DataProcessor.prototype.getTimePeriod = function() {
    var begin_time, end_time, tmp;

    tmp = this._data[0].trim().split(/[\s]/);
    begin_time = parseInt(tmp[1]);
    tmp = this._data[this._data.length - 1].trim().split(/[\s]/);
  ...
  DataProcessor.prototype._cutOffSecondFromUnixTime = function(unixtime) {
    var convert_to_unix, cut_off_second;
...
VisualizationParser = (function() {
  ...
  VisualizationParser.prototype.parseChordDiagram = function(data, filename) {
    var color_code, d, data_matrix, data_text, i, index_key, index_val, j, matrix_map, obj, unique_index, val_list, _i, _j, _k, _len, _len1, _len2;
    ...
    return {
      info: data_text,
      matrix: JSON.stringify(data_matrix)
    };
  };
...
Meteor.methods({
  uploadData: function(data) {
    ...
    if (FilesName.findOne({
      filename: data.name
    }) === void 0) {
      data.save('public/uploaded-files~/');
      filename = data.name;
      return FilesName.insert({
        filename: filename
      });
    } else {
  ...
  deleteFile: function(file_id, file_type) {
    var file, name;
  ...
  getFileData: function(file_id, file_type) {
    var data, file, name;
  ...
  getTimePeriod: function(datafile_name, posfile_name) {
    var c, processor, sd;
...
_getConvertorTimeLineFormat = function(data_file, position_file) {
  var c, sd;
...
Meteor.Router.add('/data/piechart/datafile=:data_filename&&posfile=:pos_filename', function(data_filename, pos_filename) {
  var convertor, data_piechart, position_file, processor, result, vp;
...
Meteor.Router.add('/data/timeline/datafile=:data_filename&&posfile=:pos_filename', function(data_filename, pos_filename) {
  var convertor, data_timeline, position_file, processor, result, vp;
...
Meteor.Router.add('/data/chorddiagram/info/datafile=:data_filename&&posfile=:pos_filename', function(data_filename, pos_filename) {
  var convertor, data_chorddiagram, position_file, processor, result, vp;
...
Meteor.Router.add('/data/chorddiagram/matrix/datafile=:data_filename&&posfile=:pos_filename', function(data_filename, pos_filename) {
  var convertor, data_chorddiagram, position_file, processor, result, vp;
...
this.ConvertorTimelineFormat = ConvertorTimelineFormat;
